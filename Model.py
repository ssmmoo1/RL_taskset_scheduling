...
class TaskGCN(nn.Module):
    def __init__(self, in_features, hidden_features, out_features, rel_names, mlp=False, layers=2):
        super().__init__()


        if layers == 1:
            self.sage = RGCN_1(in_features, hidden_features, out_features, rel_names)
        else:
            self.sage = RGCN_2(in_features, hidden_features, out_features, rel_names)
